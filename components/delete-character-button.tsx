...
export default function DeleteCharacterButton({ id }: { id: string }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      // Remove from store
      removeCharacter(id)
      console.log("Removed character from store:", id)
      ...
      try {
        ...
        if (response.ok) {
          console.log("Deleted character from API:", id)
        } else {
          console.warn("API deletion returned non-OK status:", response.status)
        }
      } catch (apiError) {
        console.error("API deletion failed, but character was removed locally:", apiError)
      }

      showSuccess("Character deleted successfully!")

      // Navigate to characters page after a short delay
      setTimeout(() => {
